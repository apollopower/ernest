...
func TestAgent_ConversationHistory(t *testing.T) {
	...
	router := provider.NewRouter([]provider.Provider{mp}, 30*time.Second)
	a := New(router, nil, &config.ClaudeConfig{SystemPrompt: "Be helpful"}, 0, false)
...
func TestAgent_ToolCallAndExecution(t *testing.T) {
	...
	router := provider.NewRouter([]provider.Provider{mp}, 30*time.Second)
	a := New(router, registry, &config.ClaudeConfig{}, 0, false)
...
func TestAgent_ToolPermission_AutoAllowed(t *testing.T) {
	...
	a := New(router, registry, &config.ClaudeConfig{AllowedTools: []string{"bash"}}, 0, false)
...
func TestAgent_ToolPermission_Denied(t *testing.T) {
	...
	a := New(router, registry, &config.ClaudeConfig{DeniedTools: []string{"bash"}}, 0, false)
...
func TestAgent_AllowToolAlways(t *testing.T) {
	...
	router := provider.NewRouter([]provider.Provider{mp}, 30*time.Second)
	a := New(router, registry, &config.ClaudeConfig{ProjectDir: dir}, 0, false)
